...
function buildInsight(s: any, now: Date): Insight {
  ...
  // Usa pumpState (= estado real do dispositivo de automação) em vez de
  // loadStatus (intent), pra ficar consistente com decideAction e nunca mostrar
  // uma ação que o engine na verdade não vai executar.
  const pumpState: "ON" | "OFF" | "UNKNOWN" = s.derived?.pumpState ?? "UNKNOWN";
  const automationOnline: boolean = s.state.sonoffOnline ?? false;
  ...
  if (pumpState === "UNKNOWN" || !automationOnline) {
    return {
      Icon: AlertTriangle, iconColor: "text-amber-400", tone: "warn",
      title: "Automação offline",
      subtitle: "Sem comunicação com o controlador da bomba. Sistema não pode atuar até reconectar.",
    };
  }
